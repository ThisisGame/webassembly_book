...
int ImGui::PlotExWithTips(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg,float middle_value,ImU32 up_color,ImU32 down_color,std::vector<std::string>& tips,std::string& hover_tip)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    if (values_count >= values_count_min)
    {
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            {
                if(up_color>0 || down_color>0)
                {
                    const float real_value = values_getter(data, (v1_idx + values_offset) % values_count);
                    if(real_value>middle_value)
                        col_base = up_color;
                    else
                        col_base = down_color;
                }
...
void ImGui::PlotHistogramWithColor(const char* label, const float* values, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int stride,float middle_value,ImU32 up_color,ImU32 down_color,std::vector<std::string>& tips,std::string& hover_tip)
{
    ImGuiPlotArrayGetterData data(values, stride);
    PlotExWithTips(ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, (void*)&data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size,middle_value,up_color,down_color,tips,hover_tip);
}
